  ...
  for (let i=1; i<=numero_cartas; i++) {
    try {
        ...
        retorno.push({
          ...
          'random_event_what_happens': codificarTexto(`Qué ocurre: ${response.data.random_event.what_happens.value} (${response.data.random_event.what_happens.suit})`),
          'random_event_involving': codificarTexto(`Involucra: ${response.data.random_event.involving.value} (${response.data.random_event.involving.suit})`),
          ...
          'npc_identity': codificarTexto(`Identidad: ${response.data.npc_identity.value} (${response.data.npc_identity.suit})`),
          'npc_goal': codificarTexto(`Objetivo: ${response.data.npc_goal.value} (${response.data.npc_goal.suit})`),
          'npc_notable_feature': codificarTexto(`Característica: ${response.data.npc_notable_feature.npc_notable_feature}: Oráculo de Detalle`),
          // 'npc_attitude_to_pcs': codificarTexto(`${response.data.npc_current_situation.attitude_to_pcs}`),
          'npc_attitude_to_pcs': codificarTexto('Actitud: Oráculo de Cómo'),
          ...
          'npc_conversation': codificarTexto('Conversación: Oráculo de Tema'),
          'dungeon': codificarTexto(dungeon(response.data)),
          'hex': codificarTexto(hex(response.data)),
          'dungeon_theme_how_it_looks': codificarTexto(`Cómo se ve: ${response.data.dungeon_theme.how_it_looks.value} (${response.data.dungeon_theme.how_it_looks.suit})`),
          'dungeon_theme_how_it_is_used': codificarTexto(`Cómo se utiliza: ${response.data.dungeon_theme.how_it_is_used.value} (${response.data.dungeon_theme.how_it_is_used.suit})`),
          'generic_generator_what_it_does': codificarTexto(`Qué hace: ${response.data.generic_generator.what_it_does.value} (${response.data.generic_generator.what_it_does.suit})`),
          'generic_generator_how_it_looks': codificarTexto(`Cómo se ve: ${response.data.generic_generator.how_it_looks.value} (${response.data.generic_generator.how_it_looks.suit})`),
          'generic_generator_how_significant': codificarTexto(`Importancia: ${response.data.generic_generator.how_significant}`),
...
cards.then(cards => {
  ...
  const writer = csvWriter.createObjectCsvWriter({
    ...
    header: [
      ...
      { id: 'generic_generator_what_it_does', title: 'generic_generator_what_it_does' },
      { id: 'generic_generator_how_it_looks', title: 'generic_generator_how_it_looks' },
      { id: 'generic_generator_how_significant', title: 'generic_generator_how_significant' },
    ],
  })
